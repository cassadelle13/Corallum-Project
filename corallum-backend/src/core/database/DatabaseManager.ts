...
// Database configuration for production scaling
...
export class DatabaseManager {
    ...
    async cacheExecution(execution: IRun, ttl: number = 3600): Promise<void> {
        const key = `execution:${execution.id}`;
        await this.redis.setEx(key, ttl, JSON.stringify(execution));
    }

    async getCachedExecution(id: string): Promise<IRun | null> {
        const key = `execution:${id}`;
        const cached = await this.redis.get(key);
        return cached ? JSON.parse(cached) : null;
    }

    async deleteCachedExecution(id: string): Promise<void> {
        const key = `execution:${id}`;
