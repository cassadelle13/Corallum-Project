// Единая библиотека типов узлов для фронтенда и бэкенда
...
export const CORALLUM_NODE_TYPES: NodeTypeDefinition[] = [
    // TRIGGERS
    {
        type: 'trigger',
...
// Helper functions
export const getNodeByType = (type: string): NodeTypeDefinition | undefined => {
    return CORALLUM_NODE_TYPES.find(node => node.type === type);
};

export const getNodesByCategory = (category: string): NodeTypeDefinition[] => {
    return CORALLUM_NODE_TYPES.filter(node => node.category === category);
};

export const getAllCategories = (): string[] => {
    return [...new Set(CORALLUM_NODE_TYPES.map(node => node.category))];
};
